...
const CreateParentCategory: React.FC<Props> = ({ accessToken }) => {
  ...
  const createIllustration = async (event: React.FormEvent) => {
    ...
    try {
      const response = await axios.post(CreateParentCategoryAPI(), formData, {
        headers: {
      ...
      console.error("親カテゴリの作成に失敗しました", error);
      alert("親カテゴリの作成に失敗しました");
    }
  };
  ...
  return (
    <>
      <div className="max-w-7xl m-auto">
        <h1 className="text-2xl font-bold mb-6">親カテゴリの作成</h1>
        <form
